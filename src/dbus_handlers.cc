                                           ...
                                           gpointer user_data)
{
    ...
    auto *proxy = tdbus_aupath_player_proxy_new_finish(res, &error);

    try
    {
        if(dbus_handle_error(&error, "Create AudioPath.Player proxy"))
                                           ...
                                           gpointer user_data)
{
    ...
    GError *error = nullptr;
    auto *proxy = tdbus_aupath_source_proxy_new_finish(res, &error);

    try
    {
        if(dbus_handle_error(&error, "Create AudioPath.Source proxy"))
